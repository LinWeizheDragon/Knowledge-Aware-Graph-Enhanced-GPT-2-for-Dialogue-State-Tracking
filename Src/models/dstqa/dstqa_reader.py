...
class DSTQAReader(DatasetReader):
    ...
    def _read(self, file_path: str):
        ...
        for dialog in dataset_json:
            ...
            # only last turn is used
            # if 'train' in file_path:
            #     # print('reading only last turns')
            #     instance = self.text_to_instance(dialog_idx, dialog_context, exact_match_feas, tags, utt_lens[-1:], labels[-1:],
            #                                  spans[-1:], span_labels[-1:])
            # else:
            #     # print('reading all turns')
